<template>
  <div class="cards">
    <va-card class="larger-padding col-span-12">
      ...
      <va-card-content>
        <va-popover icon="propane_tank" color="warning" message="Clique e cadastre um novo Botijão!" placement="right"
          open>
          <va-chip shadow color="primary" to="../gas">{{ t('monitora.botijao.botaoNovo') }}</va-chip>
        </va-popover>
      </va-card-content>
    </va-card>
      ...
      <template v-for="(botijao) in botijoes" :key="'item' + botijao.id">
        <va-card class="col-span-12 sm:col-span-6 md:col-span-3" stripe stripe-color="info">
          <va-card-title>
            ...
            {{ botijao.nome }}
            <va-spacer />
            <va-button size="small" icon="propane_tank" title="Visualizar o botijão" />
          </va-card-title>
